 ...
 * (including {@code null}).
 * 
 * 双向链表实现了List和Deque接口。实现了所有可选的列表操作，允许所有的元素（包括null）
 * 
 * <p>
 ...
 * 
 * 注意：LinkedList是不同步的，
 * 如果多个线程并发的访问一个LinkedList，并且至少一个线程修改链表结构。必须在外部增加同步。（结构修改是增加或者删除一个或多个元素；仅仅是设置元素的值不是结构修改。）
 * 通常是 通过自然地同步某个对象来完成封装列表。
 * 
 ...
 * access to the list:
 * 如果不存在这样的对象，这个链表应该使用Collections.synchronizedList方法包装。最好在创建的时候完成。防止意外的非同步访问
 * 
 * <pre>
 ...
 * time in the future.
 *
 * 此类的iterator返回的迭代器和listIterator方法都是快速失败的：
 * 如果在迭代器创建后的任何时间修改列表结构，除了通过迭代器的remove和add方法，迭代器将抛出ConcurrentModificationException，
 * 因此，面对并发修改后，迭代器会快速干净的失败，而不是冒风险，不确定的行为不确定的时间在未来
 * <p>
 ...
 * 通常来说，在存在不同步的并发修改的情况下，不可能做出任何严格的保证。
 * 快速失败的迭代器会尽最大努力抛出{@code ConcurrentModificationException}。
 * 因此，编写依赖于此异常的程序来确保程序的正确性是错误的：迭代器的快速失败行为应仅用于检测错误。
 * 
 * <p>
